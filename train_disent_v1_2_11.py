# like 1_2_10 with MSE swap loss and separate swap losses reported
...
swap_loss_fn = lambda x, y: tf.reduce_mean(tf.sqrt(tf.reduce_mean(tf.square(x-y), axis=(1, 2, 3))))
...
def train_G(A, B, A2B_B_s_gt, B2A_A_s_gt):
    with tf.GradientTape() as t:
        ...
        style_swap_rec_losses = [
            swap_loss_fn(s_a, s_a_swap),
            swap_loss_fn(s_b, s_b_swap)]

        cycle_swap_losses = [
            swap_loss_fn(A, A2B2A_swap), 
            swap_loss_fn(B, B2A2B_swap)]
