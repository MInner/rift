# use swapped codes instead of random
...
def train_G(A, B, A2B_B_s_gt, B2A_A_s_gt):
    with tf.GradientTape() as t:
        s_a = Es_A(A)
        s_b = Es_B(B)
        s_a_rand = tf.roll(s_a, 1, 0)
        s_b_rand = tf.roll(s_b, 1, 0)
        A2B = G_A2B_m(tf.concat([A, s_b_rand], axis=-1), training=True)
        B2A = G_A2B_m(tf.concat([B, s_a_rand], axis=-1), training=True)
        ...
        A2B_B_s = G_A2B_m(tf.concat([A, s_b], axis=-1), training=True)
        B2A_A_s = G_A2B_m(tf.concat([B, s_a], axis=-1), training=True)
