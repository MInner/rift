...
py.arg('--defence_img_noise_sigma', type=float, default=0.0)
py.arg('--defence_style_noise_sigma', type=float, default=0.0)
py.arg('--renorm_random_styles', type=bool, default=False)
py.arg('--permute_random_styles', type=bool, default=False)
...
py.arg('--style_img_indep_weight', type=float, default=0.0)
...
G_A2B = module.ResnetGenerator(input_shape=(args.crop_size, args.crop_size, img_dim + disent_dim), output_channels=img_dim)
G_B2A = module.ResnetGenerator(input_shape=(args.crop_size, args.crop_size, img_dim + disent_dim), output_channels=img_dim)
enc_A = module.ResnetGenerator(input_shape=(args.crop_size, args.crop_size, img_dim), output_channels=disent_dim)
enc_B = module.ResnetGenerator(input_shape=(args.crop_size, args.crop_size, img_dim), output_channels=disent_dim)

def get_disent_fn(model):
    def fn(x, y, **kwargs):
        c = tf.concat([x, y], axis=-1)
        out_img = model(c, **kwargs)
        return out_img
    return fn
...
def with_noise(t, sigma):
    if sigma > 0.0:
        noise = tf.random.normal(t.shape) * sigma
        return t + noise
    else:
        return t
...
def train_G(A, B):
    with tf.GradientTape() as t:
        s_a, s_b = enc_A(A, training=True), enc_B(B, training=True)
        
        if args.permute_random_styles:
            s_a_rand, s_b_rand = s_a[::-1], s_b[::-1]
        else:
            if args.renorm_random_styles:
                batch_size = tf.shape(s_a)[0]  # (batch, h, w, disent_dim)
                style_norm_a = tf.reduce_mean(tf.linalg.norm(tf.reshape(s_a, (batch_size, -1)), axis=1))
                style_norm_b = tf.reduce_mean(tf.linalg.norm(tf.reshape(s_b, (batch_size, -1)), axis=1))
            else:
                style_norm_a, style_norm_b = 1.0, 1.0
            s_a_rand = tf.random.normal((*A.shape[:-1], 1)) * style_norm_a
            s_b_rand = tf.random.normal((*A.shape[:-1], 1)) * style_norm_b

        A2B_rand = G_A2B_fn(A, s_b_rand, training=True)
        B2A_rand = G_B2A_fn(B, s_a_rand, training=True)
        
        A2B2A = G_B2A_fn(
            with_noise(A2B_rand, args.defence_img_noise_sigma), 
            with_noise(s_a, args.defence_style_noise_sigma), training=True)
        B2A2B = G_A2B_fn(
            with_noise(B2A_rand, args.defence_img_noise_sigma), 
            with_noise(s_b, args.defence_style_noise_sigma), training=True)

        s_b_rand_rec = enc_B(with_noise(A2B_rand, args.defence_img_noise_sigma))
        s_a_rand_rec = enc_A(with_noise(B2A_rand, args.defence_img_noise_sigma))

        A2A = G_B2A_fn(A, s_a, training=True)
        B2B = G_A2B_fn(B, s_b, training=True)

        A2B_d_logits = D_B(A2B_rand, training=True)
        B2A_d_logits = D_A(B2A_rand, training=True)
        ...
        # if args.style_img_indep_weight > 0:
        #     style_indep_loss = (cycle_loss_fn(s_a, s_a_z) + cycle_loss_fn(s_b, s_b_z))
        #     G_losses.append(args.style_img_indep_weight * style_indep_loss)
    ...
    return A2B_rand, B2A_rand, summary
...
def train_D(A, B, A2B, B2A):
    with tf.GradientTape() as t:
        ...
        if args.guess_loss_weight > 0:
            ...
            A2B2A = G_B2A_fn(
                with_noise(A2B, args.defence_img_noise_sigma), 
                with_noise(s_a, args.defence_style_noise_sigma), training=True)
            B2A2B = G_A2B_fn(
                with_noise(B2A, args.defence_img_noise_sigma), 
                with_noise(s_b, args.defence_style_noise_sigma), training=True)
...
def sample(A, B):
    ...
    A2B_rand_s = G_A2B_fn(A, s_b_rand, training=False)
    B2A_rand_s = G_B2A_fn(B, s_a_rand, training=False)
    A2B2A = G_B2A_fn(A2B_rand_s, s_a, training=False)
    B2A2B = G_A2B_fn(B2A_rand_s, s_b, training=False)
    A2B2A_perm = G_B2A_fn(A2B_rand_s, s_a[::-1], training=False)
    B2A2B_perm = G_A2B_fn(B2A_rand_s, s_b[::-1], training=False)
    A2B2A_rand = G_B2A_fn(A2B_rand_s, s_a_rand, training=False)
    B2A2B_rand = G_A2B_fn(B2A_rand_s, s_b_rand, training=False)
