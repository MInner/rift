# like 1_2_1 but with zero-based styles during eval
...
def train_G(A, B, A2B_B_s_gt, B2A_A_s_gt):
    with tf.GradientTape() as t:
        ...
        s_a_zero = G_A2B_fn(A, s_zero, training=True)[1]
        s_b_zero = G_B2A_fn(B, s_zero, training=True)[1]
        s_a_rand, s_b_rand = [tf.roll(x, 1, 0) for x in [s_a_zero, s_b_zero]]
        ...
        style_zero_rec = (
            cycle_loss_fn(s_a, s_a_zero) 
            + cycle_loss_fn(s_b, s_b_zero))
...
def sample_nzs(A, B):
    ...
    A2B2A_perm, _ = G_B2A_fn(A2B_rand_s, s_a_perm, training=False)
    B2A2B_perm, _ = G_A2B_fn(B2A_rand_s, s_b_perm, training=False)
...
@tf.function
def sample_grid_nzs(A, B):
    bs = A.shape[0]
    s_zero = tf.zeros((*A.shape[:-1], 1))
    s_a_zero = G_A2B_fn(A, s_zero, training=True)[1]
    s_b_zero = G_B2A_fn(B, s_zero, training=True)[1]
    s_a_rand, s_b_rand = [tf.roll(x, 1, 0) for x in [s_a_zero, s_b_zero]]
    s_a = G_A2B_fn(A, s_b_rand, training=False)[1]
    s_b = G_B2A_fn(B, s_a_rand, training=False)[1]
    A_rep, B_rep = [tf.repeat(x, bs, 0) for x in [A, B]]
    s_a_tile, s_b_tile = [tf.tile(x, (bs, 1, 1, 1)) for x in [s_a, s_b]]
    A2B_B_s_grid_flat = G_A2B_fn(A_rep, s_b_tile, training=False)[0]
    B2A_A_s_grid_flat = G_B2A_fn(B_rep, s_a_tile, training=False)[0]
...
def get_total_grid(A, B, A2B_B_s_grid):
  draw_col = A.numpy().reshape(8*64, 64, 3)
  draw_row = B.numpy().swapaxes(0, 1).reshape(64, 8*64, 3)
  draw_grid = A2B_B_s_grid.numpy().swapaxes(1, 2).reshape(64*8, 64*8, 3)
  total_grid_items = [[np.zeros((64, 64, 3)), draw_row], [draw_col, draw_grid]]
  total_grid = np.concatenate([np.concatenate(x, axis=1) for x in total_grid_items], axis=0)
  return total_grid


def get_AB_nzs_grids(A, B):
    A2B_B_s_grid, B2A_A_s_grid = sample_grid_nzs(A, B)
    ...
    for ep in tqdm.trange(args.epochs, desc='Epoch Loop'):
        ...
        for A, B, A2B_B_s_gt, B2A_A_s_gt in tqdm.tqdm(A_B_dataset, desc='Inner Epoch Loop', total=len_dataset):
            ...
            if G_optimizer.iterations.numpy() % args.log_img_every == 0:
                ...
                for sample_name, sample_fn in zip(['nzs', 'zs'], [sample_nzs, sample_zs]):
                    metrics = {
                    ...
                    for img_idx in range(args.write_n_imgs):
                        ...
                        output_sample_dict = sample_fn(A, B)
                        ...
                        if sample_name == 'nzs':
                            grid_imgs.append(get_AB_nzs_grids(A, B))

                        if args.write_imgs_to_disk:
                            fn = '%s-iter-%09d-%d.jpg' % (sample_name, G_optimizer.iterations.numpy(), img_idx)
                            im.imwrite(img, py.join(sample_dir, fn))

                    if args.save_img_to_tb:
                        tf.summary.image(
                            'img-%s' % sample_name, imgs, 
                            step=G_optimizer.iterations, max_outputs=args.write_n_imgs)
                    ...
                    for split_name, split_metrics in metrics.items():
                        for cmp_name, attr_metrics in split_metrics.items():
                            for attr_name, metric in attr_metrics.items():
                                name = 'attr-%s/%s-%s-%s' % (sample_name, split_name, cmp_name, attr_name)
