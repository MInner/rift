# like 1_2_1 + cycle recs from generator and not pool
...
py.arg('--seed', type=int, default=10)
py.arg('--pool_size', type=int, default=50)  # pool size to store fake samples

args = py.args()
tf.random.set_seed(args.seed)
np.random.seed(args.seed)  # backported from 2_2
py_rand_seed = random.Random(args.seed)
...
py_rand_seed.shuffle(dataset_file_rows)
...
def train_D(A, B, A2B, B2A, A2B2A, B2A2B):
    with tf.GradientTape() as t:
        ...
        if args.guess_loss_weight > 0:
            ...
            D_losses.append(
                (A_fwd_guess_loss + A_rev_guess_loss 
                 + B_fwd_guess_loss + B_rev_guess_loss) * args.guess_loss_weight
            )
