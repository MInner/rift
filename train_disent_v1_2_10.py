# like 1_2_9 with trans cycle on A not A2B2A as second arg
...
py.arg('--gt_attr_clf_input_size', default='')
py.arg('--is_categorical_attr', default='')
...
py.arg('--style_trans_rec_weight', type=float, default=0.0)
py.arg('--cycle_trans_weight', type=float, default=0.0)
...
dataset_defaults = {
    'dshapes_split_color_vs_ori_size': {
        ...
        'gt_attr_clf_model_path': '/scratch3/run/honest_disent/attr_models/shape_v2/',
        'is_categorical_attr': {
    ...
    'synaction_bg_vs_idt_v3': {
        ...
        'gt_dataset_fn': 'gt.txt',
        'gt_dataset_img_folder': 'all',
        'gt_attr_clf_model_path': '/scratch3/run/honest_disent/attr_models/synaction3_v2/',
        'is_categorical_attr': {
            'idt': True, 'bg': True
        },
    ...
    'celeba_custom_split': {
        'gt_dataset_root': '/scratch3/data/guess_disent/celeba_custom_split/',
...
def train_G(A, B, A2B_B_s_gt, B2A_A_s_gt):
    with tf.GradientTape() as t:
        ...
        s_a_tr = G_A2B_fn(B2A_A_s, s_zero, training=True)[1]
        s_b_tr = G_B2A_fn(A2B_B_s, s_zero, training=True)[1]
        
        A2B2A_tr = G_B2A_fn_w_nz(A2B, s_a_tr)[0]
        B2A2B_tr = G_A2B_fn_w_nz(B2A, s_b_tr)[0]
        ...
        style_trans_rec_loss = (
            cycle_loss_fn(s_a, s_a_tr) 
            + cycle_loss_fn(s_b, s_b_tr))

        cycle_trans_loss = (
            cycle_loss_fn(A, A2B2A_tr) 
            + cycle_loss_fn(B, B2A2B_tr))
        ...
        if args.style_trans_rec_weight > 0:
            G_losses.append(style_trans_rec_loss * args.style_trans_rec_weight)

        if args.cycle_trans_weight > 0:
            G_losses.append(cycle_trans_loss * args.cycle_trans_weight)
    ...
    summary = {
        ...
        'style_trans_rec_loss': style_trans_rec_loss,
        'cycle_trans_loss': cycle_trans_loss,
        'total': G_loss,
    }
...
def sample_zs(A, B):
    ...
    A2B2A_tr = G_B2A_fn_w_nz(A2B_rand_s, s_a_tr)[0]
    B2A2B_tr = G_A2B_fn_w_nz(B2A_rand_s, s_b_tr)[0]

    names = ['A2B', 'B2A', 'A2B2A', 'B2A2B', 'A_perm', 'B_perm', 
            'A2B2A_perm', 'B2A2B_perm', 'A2B2A_rand', 'B2A2B_rand',
            'A2B_B_s', 'B2A_A_s', 'A2B2A_tr', 'B2A2B_tr']
        ...
        for A, B, A2B_B_s_gt, B2A_A_s_gt in tqdm.tqdm(A_B_dataset, desc='Inner Epoch Loop', total=len_dataset):
            ...
            if G_optimizer.iterations.numpy() % args.log_img_every == 0:
                ...
                for img_idx in range(args.write_n_grid_imgs):
                    ...
                    img_names_to_merge = [
                        'A', 'A2B', 'A2B2A', 'A2B2A_rand', 'A_perm', 'A2B2A_perm', 'A2B_B_s', 'A2B_B_s_gt', 'A2B2A_tr',
                        'B', 'B2A', 'B2A2B', 'B2A2B_rand', 'B_perm', 'B2A2B_perm', 'B2A_A_s', 'B2A_A_s_gt', 'B2A2B_tr']
