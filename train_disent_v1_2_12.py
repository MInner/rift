# like 1_2_22 with max swap loss on images
...
sty_swap_loss_fn = lambda x, y: tf.reduce_mean(tf.sqrt(tf.reduce_mean(tf.square(x-y), axis=(1, 2, 3))))
img_swap_loss_fn = lambda x, y: tf.reduce_mean(tf.reduce_max(tf.abs(x-y), axis=(1, 2, 3)))
